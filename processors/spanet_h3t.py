...
def jetSelection(jetDict):
    ...
    seq.append(
        EventSkim(selection=lambda event, systNames=systNames: 
            any([getattr(event, "nselectedJets_"+systName) >= 9 for systName in systNames])
        )
    )
    ...
    seq.append(
        EventSkim(selection=lambda event, systNames=systNames: 
            any([len(filter(lambda jet: jet.isBTagged,getattr(event,"selectedJets_"+systName))) >= 3 for systName in systNames])
        )
    )
    '''
    #at least 2 AK8 jets
