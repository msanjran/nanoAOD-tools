...
def leptonSequence():
    seq = [
        ...
        EventSkim(selection=lambda event: (event.IsoMuTrigger_flag == 1) or (event.IsoElectronTrigger_flag == 1)),
...
def jetSelection(jetDict):
    ...
    seq.append(
        EventSkim(selection=lambda event, systNames=systNames: 
            any([getattr(event, "nselectedJets_"+systName) >= 5 for systName in systNames])
        )
    )
